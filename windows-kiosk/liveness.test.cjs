...
test('challenge rejects and consumes pre-issuance, future, and non-monotonic timestamps', () => {
  ...
  const beforeResult = manager.validateAndConsume('t-before', { challengeId: before.id, observations: frames([0, -.35, 0, .35, 0], 1000) });
  assert.equal(beforeResult.ok, false);
  assert.equal(manager.validateAndConsume('t-before', { challengeId: before.id, observations: frames([0, -.35, 0, .35, 0], 10_000) }).ok, false);
  assert.equal(manager.getFailureState('t-before').count, 2);

  const future = manager.begin('t-future');
  const futureResult = manager.validateAndConsume('t-future', { challengeId: future.id, observations: frames([0, .35, 0, -.35, 0], 20_000) });
  assert.equal(futureResult.ok, false);
  assert.equal(manager.validateAndConsume('t-future', { challengeId: future.id, observations: frames([0, .35, 0, -.35, 0], 10_000) }).ok, false);
  ...
  assert.equal(manager.validateAndConsume('t-non-monotonic', { challengeId: nonMonotonic.id, observations: frames([0, -.35, 0, .35, 0], 10_000) }).ok, false);
...
test('cancel consumes an incomplete challenge and counts failure', () => {
  ...
  assert.equal(manager.validateAndConsume('t1', { challengeId: challenge.id, observations: frames([0, -.35, 0, .35, 0], 1000) }).ok, false);
});

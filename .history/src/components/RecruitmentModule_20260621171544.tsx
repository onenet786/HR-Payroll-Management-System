...
export function RecruitmentModule({
  ...
  const handleAddJob = () => {
    ...
    onAddPosting({ id: `job-${Date.now()}`, ...(jobForm as JobPosting) });
  ...
  const handleAddApp = () => {
    ...
    onAddApplication({ id: `app-${Date.now()}`, ...(appForm as JobApplication) });
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              ...
              <div className="flex justify-between items-start">
                ...
                <button onClick={() => setAppDetailId(null)} className="text-slate-500 hover:text-white transition"><X size={18} /></button>
              </div>

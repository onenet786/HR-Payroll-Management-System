          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Review Period</label>
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Review Date</label>
                ...
                <div className="space-y-2">
                  {form.kpis.map((kpi, idx) => (
                    <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                      <input value={kpi.kpi} onChange={e => updateKpi(idx, 'kpi', e.target.value)}
                        placeholder="KPI Name" className="col-span-4 bg-slate-800 border border-slate-700 text-white text-xs rounded-lg px-2 py-1.5 focus:outline-none" />
                      <div className="col-span-1 flex items-center gap-1">
                        <input type="number" min="5" max="100" value={kpi.weight} onChange={e => updateKpi(idx, 'weight', Number(e.target.value))}
              ...
              <div className="flex items-center gap-4 bg-slate-800/60 rounded-xl p-3">
                ...
                {form.incrementRecommended && (
                  <div className="flex items-center gap-2">
                    <input type="number" min="0" max="100" value={form.incrementPercent}

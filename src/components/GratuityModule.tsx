          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label htmlFor="gratuity-employee" className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Employee *</label>
                  <select id="gratuity-employee" value={calcForm.employeeId} onChange={e => setCalcForm(p => ({ ...p, employeeId: e.target.value }))}
                ...
                <div>
                  <label htmlFor="gratuity-separation-type" className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Separation Type</label>
                  <select id="gratuity-separation-type" value={calcForm.separationType} onChange={e => setCalcForm(p => ({ ...p, separationType: e.target.value as SeparationType }))}
                ...
                <div>
                  <label htmlFor="gratuity-last-working-date" className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Last Working Date</label>
                  <input id="gratuity-last-working-date" type="date" value={calcForm.lastWorkingDate}
                ...
                {[
                  ...
                  <div key={key}>
                    <label htmlFor={`gratuity-${key}`} className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{label}</label>
                    <input id={`gratuity-${key}`} type="number" min="0" value={(calcForm as any)[key]}
                      onChange={e => setCalcForm(p => ({ ...p, [key]: Number(e.target.value) }))}
                      className="w-full bg-slate-800 border border-slate-700 text-white text-sm rounded-xl px-3 py-2.5 focus:outline-none" />
                  </div>
                ))}
                <div>
                  <label htmlFor="gratuity-notes" className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Notes</label>
                  <input id="gratuity-notes" value={calcForm.notes} onChange={e => setCalcForm(p => ({ ...p, notes: e.target.value }))}
